...
import com.dekastudio.fragment.fragmentpenyakit.FragmentCiriCiri;
import com.dekastudio.fragment.fragmentpenyakit.FragmentInfo;
import com.dekastudio.fragment.fragmentpenyakit.FragmentMenangani;
...
public class DetailPenyakit extends AppCompatActivity {
    ...
    private ModelPenyakit modelPenyakit;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        idPenyakit = getIntent().getIntExtra(Constant.INDEX_CODE, -1);
        modelPenyakit = databaseHelper.bacaDataDenganId(new ModelPenyakit(idPenyakit));

        //baca model tanaman yg dapat terjangkit hama ini
        modelTanaman = databaseHelper.bacaDataDenganId(new ModelTanaman(modelPenyakit.getIdTanaman()));

        //area tab layout
        ViewPager viewPager = findViewById(R.id.viewpager);
        DetailPenyakitTabAdapter detailPenyakitTabAdapter = new DetailPenyakitTabAdapter(getSupportFragmentManager());
        viewPager.setAdapter(detailPenyakitTabAdapter);
        ...
        for (ModelFotoPenyakit fotoPenyakit: ModelFotoPenyakit.bacaFotoPenyakit(databaseHelper.getOpenDatabase(), idPenyakit)) {
            AppCompactImageSlider defaultSliderView = new AppCompactImageSlider(this);
            defaultSliderView.image(ImageHelper.getUrl(((AppCore)getApplication()).getRequestQueue(), Constant.PENYAKIT_FOLDER, fotoPenyakit.getNamaFile()));
        ...
        ImageView bookmark = findViewById(R.id.btnBookmarkPenyakit);
        if(modelPenyakit.getBookmark() == 1){
            bookmark.setImageResource(R.drawable.ic_bookmark_on);
        }
        bookmark.setOnClickListener(v -> {
            if(modelPenyakit.getBookmark() == 1){
                //hapus bookmark
                modelPenyakit.setBookmark(0);
                databaseHelper.updateData(modelPenyakit, modelPenyakit.getId());
                bookmark.setImageResource(R.drawable.ic_bookmark_off);
                Toast.makeText(this, "Dihapus dari bookmark", Toast.LENGTH_SHORT).show();
                unBookmark = true;
            }else{
                //simpan bookmark
                modelPenyakit.setBookmark(1);
                databaseHelper.updateData(modelPenyakit, modelPenyakit.getId());
                bookmark.setImageResource(R.drawable.ic_bookmark_on);
    ...
    public ModelPenyakit getModelPenyakit() {
        return modelPenyakit;
    }

    public ModelTanaman getModelTanaman() {
        return modelTanaman;
    }
    ...
    public void onBackPressed() {
        ...
        data.putExtra(Constant.INDEX_CODE, idPenyakit);
        if(unBookmark) {
    ...
    class DetailPenyakitTabAdapter extends FragmentPagerAdapter {

        private final Fragment[] daftarFragment = {new FragmentInfo(), new FragmentCiriCiri(), new FragmentMenangani()};
        private final String[] daftarJudulTab = {"Info", "Ciri-Ciri", "Menangani"};

        DetailPenyakitTabAdapter(FragmentManager manager) {
            super(manager);
        }
